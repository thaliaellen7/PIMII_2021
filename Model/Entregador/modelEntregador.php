...
class controlEntregador {
	...
	public static function listarPedidosProntos () {
		...
		$listado = $conexao->consultar("SELECT * FROM pedidos WHERE status in ('Pronto','Entregue','A caminho') AND idEmpresa = '".$_SESSION['idEmpresa']."' AND data LIKE '".$dataAtual."%' ORDER BY idPedidos DESC");
		$conexao->encerrar();
		return $listado;
	}
